	...
	SubShader{
		Pass {
			...
			float sdScene(float3 p) {
				p /= 0.8;// The uniform scale of the whole scene is 0.8 times
				p = mul(rotateY(_Time[1]*0.5), p);// The whole scene rotates around the Y axis
				float d1 = sdRoundBox(p, float3(2.0, 1.0, 2.0), 0.1);
				float d2 = sdSphere(p - float3(0.0, 1.0 + 1.0 * sin(_Time[1]), 0.0), 1.0);// Sphere translation
				return opSubtraction(d2, d1) * 0.8;// Take the smooth intersection, *0.8 represents the compensation of the uniform scale in the first step
			}
			...
			fixed4 frag(v2f i) : SV_Target{
				float2 scr_coords = (i.scrPos.xy / i.scrPos.w) * _ScreenParams.xy;//The screen coordinates of the fragment
				...
				fixed3 dir = rayDirection(fov, _ScreenParams.xy, scr_coords);// Calculate the raymarching direction of the pixel (view space)
				fixed3x3 view2world = viewMatrix(eyePos, centerPos, up);// Calculate the view to world vector transformation matrix
				fixed3 worldDir = mul(view2world, dir);// Convert the raymarching direction of view space to world space
				...
				float3 p = eyePos + dist * worldDir;// Calculate the coordinates of the point where raymarching intersects the scene

				// phong shading params
				fixed3 K_a = fixed3(0.0, 0.0, 0.0);// ambient color
				fixed3 K_d = fixed3(1.0, 1.0, 1.0);// diffuse color
				fixed3 K_s = fixed3(0.0, 0.0, 0.0);// specular color
				float shininess = 0.0;
